...
class generator(nn.Module):
    def __init__(self, opts):
        ...
        self.dim_z = opts.nz
        self.dim = opts.img_size
        self.img_size = opts.img_size
        self.ssize = self.dim // 16
        self.ln1 = nn.Linear(self.dim_z, self.ssize*self.ssize*8*self.dim)
        ...
        self.conv1 = MyConvo2d(self.dim, 3, 3)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()

    def forward(self, input):
        output = self.ln1(input)
        # output = output.view(-1, self.dim_z, self.ssize, self.ssize)
        output = output.view(-1, self.dim*8, self.ssize, self.ssize)
    ...
    def change_z_num(self, next_z):
        self.ln1 = nn.Linear(next_z, self.ssize * self.ssize * self.dim*8)
...
class discriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        output = self.rb3(output)
        output = self.rb4(output)
        output = output.view(-1, self.ssize*self.ssize*self.dim*8)
