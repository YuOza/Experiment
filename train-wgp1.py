...
from inception_score import inception_score_cal
...
def train_one_time(nz, real_z, n_ep, m_name, dataset_name, value, space):
    ...
    for ep in range(ep0, opts.n_ep):
        ...
        if ep % space == 0 and ep != 0:
            ...
            generated_dataset = datasets.ImageFolder(root=Gpath, transform=data_transform)
            now_is, v = inception_score_cal(generated_dataset, cuda=True, batch_size=32, resize=True, splits=10)
            #--------
            model.train()
            islist.append(now_is)
            if highest_is < now_is: # 最良が更新された
